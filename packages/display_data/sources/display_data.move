// On-chain Metadata program
// Do we like the term 'Display data' or 'Metadata' better?

module sui_playground::display_data {
    ...
    struct DisplayData<phantom T> has key, store {
        id: UID,
        module_addr: String,
        module_name: String
    }
    ...
    // There is probably a better way to create Singletons than using a one-time-witness pattern
    public fun create_display_data<T: drop>(witness: &T, ctx: &mut TxContext): DisplayData<T> {
    ...
    public fun add_data<T, Object: key, D: store + drop>(display: &mut DisplayData<T>, object_display: D) {
        remove_data<T, Object, D>(display);
        let (_, _, key) = encode::type_name_<Object>();
        dynamic_field::add(&mut display.id, key, object_display);
    }

    public entry fun remove_data<T, Object: key, D: store + drop>(display: &mut DisplayData<T>) {
